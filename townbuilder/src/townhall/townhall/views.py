...
def datasets_list(request):
    ...
    try:
        ...
        print(f"townhall.datasets_list: could not query DatasetFileModel: {e}")
        all_files = []
...
def remote_file_browser_html(dir_current, sort_field=None, sort_dir='asc', extra_columns=None, browser_url_name='remote_file_browser'):
    ...
    context = {
        ...
        'browser_url': django.urls.reverse(f'townhall:{browser_url_name}'),
    }

    html = render_to_string('partial_remote_file_browser.html', context)
    return html


@hx_or_full("base.html")
    ...
    extra_columns = [
        {
            ...
            'field': None,
            'sortable': False,
            'template': 'partial_add_file_button.html'
        }
    ]
...
def plot_matplotlib_example(request):
    ...
    html = render_to_string('partial_single_matplotlib_plot.html', {'chart_html': chart_html,
...
def log_viewer(request):
    ...
    return render_to_string('partial_logs.html')
...
def log_entries(request):
    ...
    if start <= 0:
        html = render_to_string('partial_log_results.html', {
            'logs': page_msgs,
            'has_more': has_more,
            'next_url': next_url,
        })
    else:
        html = render_to_string('partial_log_rows.html', {
            'logs': page_msgs,
            'has_more': has_more,
            'next_url': next_url,
        })
...
def log_levels(request):
    ...
    html = render_to_string('partial_log_levels.html', {'log_levels': levels})
    return HttpResponse(html)
...
def log_loggers(request):
    ...
    html = render_to_string('partial_log_loggers.html', {'loggers': sorted_loggers})
    return HttpResponse(html)
...
def upload_datasets(request):
            ...
            if os.path.isdir(folder_path):
                ...
                for name in sorted(os.listdir(folder_path)):
                    ...
                    if ext not in ['.db', '.sqlite', '.sqlite3']:
                        continue
                    metadata = load_dataset_metadata(full_path)
                    label = metadata.get('dataset_name', name)
                    obj, created = models.DatasetFileModel.objects.get_or_create(
                        filepath=full_path,
                        defaults={'is_open': False, 'label': label}
                    )
                    if created and not obj.label and metadata.get('dataset_name'):
                        obj.label = metadata.get('dataset_name')
                        obj.save()
                    found += 1
            ...
            for uploaded_file in uploaded_files:
                ...
                with open(temp_path, 'wb') as f:
                    for chunk in uploaded_file.chunks():
                        f.write(chunk)
                metadata = load_dataset_metadata(temp_path)
                label = metadata.get('dataset_name', uploaded_file.name)
                obj, created = models.DatasetFileModel.objects.get_or_create(
                    filepath=temp_path,
                    defaults={'is_open': False, 'label': label}
                )
    ...
    try:
        ...
        print(f"townhall.open_database_files: could not query DatasetFileModel: {e}")
        available_datasets = []

    html = render_to_string('partial_dataset_manager.html', {
...
def add_dataset_file(request):
    ...
    if request.method == 'POST':
        ...
        metadata = load_dataset_metadata(filepath)
        label = metadata.get('dataset_name')
        ...
        if not created:
            ...
            if not file_obj.label and label:
