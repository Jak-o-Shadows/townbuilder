...
systems::systems(flecs::world& ecs) {
    ...
    systemsLogger->trace("Modules Imported");


    // Setup window
	glfwSetErrorCallback(glfw_error_callback);
	if (!glfwInit()) {
        systemsLogger->error("Could not init glfwInit");
		for (;;) {};
	}
	...
	if (window == NULL) {
        systemsLogger->error("GLFWWindow null");
		for (;;) {};
	}
	...
	if (err)
	{
        systemsLogger->error("Failed to initialize OpenGL loader!");
		for (;;) {};
	}
    ...
    ecs.system("glfwWindowPollEvents")
        ...
        systemsLogger->trace("glfwWindowPollEvents");

		glfwPollEvents();
		glClearColor(0.45f, 0.55f, 0.60f, 1.00f);
		glClear(GL_COLOR_BUFFER_BIT);
        systemsLogger->trace("glClear");

		// feed inputs to dear imgui, start new frame
		ImGui_ImplOpenGL3_NewFrame();
		ImGui_ImplGlfw_NewFrame();
		ImGui::NewFrame();
        systemsLogger->trace("ImGui NewFrame");
        });
    ...
    ecs.system("ExampleImGuiDrawing")
        ...
        systemsLogger->trace("ExampleImGuiDrawing");
        ...
        render_conan_logo();
        ImGui::End();
        systemsLogger->trace("ImGui Render Conan Logo");
    ...
    ecs.system<Window>("RenderImGui")
        ...
        systemsLogger->trace("ImGuiRenderDrawData");

		int display_w, display_h;
		glfwGetFramebufferSize(w.window, &display_w, &display_h);
		glViewport(0, 0, display_w, display_h);
		glfwSwapBuffers(w.window);
        systemsLogger->trace("glfwSwapBuffers");
        });
        ...
        .run([](flecs::iter& it){
            ...
            while (it.next()) {
                ...
                for (auto i : it) {
                    ...
                    systemsLogger->trace("Rendered {} of: {}, {}, {}, {}", std::string(e.path()), corner1.x, corner1.y, corner3.x, corner3.y);
                }
    ...
    ecs.observer<>("Granary_BuildingCreateAddRenderComponents")
        .event(flecs::OnAdd)
        .with(flecs::IsA).second<Buildings::Granary_Prefab>()
        //.with<Buildings::Granary_Prefab>(flecs::IsA)
        .each([](flecs::entity e){
            ...
            systemsLogger->debug("Added Render Components to Building {}", std::string(e.path()));
        });
    ...
    ecs.observer<>("Tree_CreateAddRenderComponents")
        ...
        .each([](flecs::entity e){
            ...
            systemsLogger->debug("Added Render Components to Tree {}", std::string(e.path()));
        });
    ...
    ecs.observer<const Map::GridCellStatic>("GridCell_CreateAddRenderComponents")
        ...
        .each([](flecs::entity e, const Map::GridCellStatic pos){
            ...
            systemsLogger->debug("Added Render Components to GridCell {}", std::string(e.path()));
        });
